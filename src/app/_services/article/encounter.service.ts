...
export class EncounterService extends BaseService<Encounter> {
  ...
  createForDiaryentry(encounter: Partial<Encounter>): Observable<Encounter[]>{
    ...
    return this.http.post<any[]>(url, encounter)
      .pipe(
        map(entries => entries.map(entry => this.parseEntity(entry))),
      );
  }

  swapEncounterOrder(campaign: string, encounter1_pk: number, encounter2_pk: number): Observable<Encounter[]>{
  ...
  cutInsertEncounter(campaign: string, encounter: Encounter, newOrderIndex: number): Observable<Encounter[]>{

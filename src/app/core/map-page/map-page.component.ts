...
import { selectCurrentCampaignName, selectMap, selectMapOverviewItems } from '../app.reducer';
...
export class MapPageComponent implements OnInit{
  ...
  ngOnInit(): void {
    const hasMemberPermissions$: Observable<boolean> = this.store
      .select(selectCurrentCampaignName)
      .pipe(map(name => this.tokenService.isCampaignMember(name as string)));;
  ...
  mapChange(map: OverviewItem): void{
    ...
    this.store.select(selectCurrentCampaignName)
      .pipe(first())

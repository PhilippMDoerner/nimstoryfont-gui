...
import { ConfigTableKind } from "src/app/design/templates/_models/config-table";
import { createConfigTableEntry, createConfigTableEntryFailure, createConfigTableEntrySuccess, deleteConfigTableEntry, deleteConfigTableEntryFailure, deleteConfigTableEntrySuccess, loadConfigTableEntries, loadConfigTableEntriesFailure, loadConfigTableEntriesSuccess } from "../app.actions";
  ...
  load$: Observable<Action> = createEffect((): Observable<Action> => this.actions$.pipe(
    ...
    switchMap(({ table }): Observable<Action> => {
      ...
      return service.list().pipe(
        switchMap((entries: unknown[]) => of(loadConfigTableEntriesSuccess({table, entries: entries}))),
        catchError(error => of(loadConfigTableEntriesFailure(error)))
      );
    }),
  ));
  
  deleteEntry$: Observable<Action> = createEffect((): Observable<Action> => this.actions$.pipe(
    ofType(deleteConfigTableEntry),
    switchMap(({ table, entryId }): Observable<Action> => {
      const service = this.getService(table);
      return service.delete(entryId).pipe(
        switchMap(() => of(deleteConfigTableEntrySuccess({ table, entryId }))),
        catchError(error => of(deleteConfigTableEntryFailure(error)))
      );
    }),
  ));
  
  createEntry$: Observable<Action> = createEffect((): Observable<Action> => this.actions$.pipe(
    ofType(createConfigTableEntry),
    switchMap(({table, entry}): Observable<Action> => {
      const service = this.getService(table);
      return service.create(entry).pipe(
        switchMap((newEntry) => of(createConfigTableEntrySuccess({table, entry: newEntry}))),

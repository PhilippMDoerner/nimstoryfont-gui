...
import * as Leaflet from 'tinymce';
...
export class LeafletMapComponent implements AfterContentInit{
  ...
  setMapEventListeners() {
    ...
    this.leafletMap.on('mousemove', (event: any) => {
  ...
  addMapImage(): void {
    ...
    L.imageOverlay(this.serverUrl + this.map.image, bounds).addTo(this.leafletMap);
  ...
  addMarkers(): void {
    ...
    for (let mapMarker of this.map.markers ?? []) {
      ...
      const layerWithNameExists = layerName && layers.hasOwnProperty(layerName);
      const layer = layerWithNameExists ? layers[layerName] : L.layerGroup();
  ...
  makeLocationHeading(marker: MapMarker): string {
  ...
  makeLocationDescription(marker: MapMarker): string {
  ...
  makeSublocationList(marker: MapMarker): string {

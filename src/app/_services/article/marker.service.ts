...
export class MarkerService extends BaseService<MapMarker>{
  ...
  private generateUrlCallback(data: any){
    ...
    const parent_location_name = data.parent_location_name ?? data.parent_location_details?.name ?? data.location_details.parent_location_name; 

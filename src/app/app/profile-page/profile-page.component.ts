...
export class ProfilePageComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.isCurrentUser$ = this.user$.pipe(
      map(user => user.pk === userId),
    );
    
    this.campaignName$ = this.store.select(selectCurrentCampaignName)
  ...
  private setMemberships(): void{
    const memberships: CampaignMemberships = this.tokenService.getCampaignMemberships();
    const campaignNames =  Object.keys(memberships); 
    this.memberships = campaignNames.map(
      name => ({
        campaignName: name, 

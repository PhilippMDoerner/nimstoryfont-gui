...
export class GeneralOverviewComponent implements OnInit, OnChanges {
  ...
  private setDiaryEntryEntries(): void{
    this.displayEntries = this.entries.map(
      diaryEntry => ({
        ...diaryEntry, 
  ...
  private setLocationEntries(): void{
    this.displayEntries = this.entries.map(
      (locationEntry: OverviewItem) => {
        const parents: OverviewItem[] = this.getParentLocations(locationEntry, this.entries);
        const parentNames: string[] = parents.map((location: OverviewItem) => location.name).reverse();
      ...
      .sort((location1: OverviewItem, location2: OverviewItem) => (location1.name_full > location2.name_full) ? 1 : -1)
    ;
  }
  
  
  private getParentLocations(location: OverviewItem, listItems: OverviewItem[]): OverviewItem[]{
    ...
    while(hasParentLocation){ // aka hasParentLocation
      const parentLocationPk: number = currentLocation.parent_location_details?.pk as number;
      const parentLocation: OverviewItem | undefined = listItems.find((entry => entry.pk === parentLocationPk));
  ...
  private padNumber(num: number, padCount: number, paddingCharacter = "0"): string{

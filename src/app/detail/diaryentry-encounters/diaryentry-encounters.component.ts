...
import { Encounter, EncounterConnection, EncounterObject } from 'src/app/_models/encounter';
...
export class DiaryentryEncountersComponent implements OnInit, OnChanges{
  ...
  setDiaryEntryEncounters(){
    this.diaryEntryEncounters = this.encounters.map(
      encounter => ({ encounter, isUpdating: false})
    );
  ...
  async swapEncounters(encounterIndex1: number, encounterIndex2: number): Promise<void>{
    ...
    const encounter1: Encounter = this.diaryEntryEncounters[encounterIndex1].encounter;
    const encounter2: Encounter = this.diaryEntryEncounters[encounterIndex2].encounter;
  ...
  private sortEncounters(){
    this.diaryEntryEncounters.sort((
      encounter1: DiaryEntryEncounter, 
      encounter2: DiaryEntryEncounter
    ) => {
  ...
  private removeEncounter(removalIndex: number){
    const cardElements: HTMLElement[] = this.elementRef.nativeElement.querySelectorAll('app-card');
    const cardElement: HTMLElement = cardElements[removalIndex];
    
    animateElement(cardElement, 'zoomOut')
      .then(() => {

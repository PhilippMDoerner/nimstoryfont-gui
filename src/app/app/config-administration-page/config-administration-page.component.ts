...
import { ConfigTableData, ConfigTableKind } from 'src/app/design/templates/_models/config-table';
import { createConfigTableEntry, deleteConfigTableEntry, loadConfigTableEntries } from '../app.actions';
...
export class ConfigAdministrationPageComponent implements OnInit{
  ...
  deleteTableEntry(event: {table: ConfigTableKind, entry: any}): void{
    this.store.dispatch(deleteConfigTableEntry({
  ...
  createTableEntry(event: {table: ConfigTableKind, entry: unknown}): void{
    this.store.dispatch(createConfigTableEntry({
      table: event.table,
      entry: event.entry
    }));
  }

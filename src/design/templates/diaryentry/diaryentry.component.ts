...
export class DiaryentryComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.overviewUrl = this.routingService.getRoutePath('diaryentry-overview', {
  ...
  private setUrl(): void {
    ...
    const campaignName = this.diaryentry.campaign_details.name;
    const sessionNumber: number = this.diaryentry.session_details
      ?.session_number as number;
    const authorName: string = this.diaryentry.author_details?.name as string;
    const isMainSession: number = this.diaryentry.session_details
      ?.is_main_session_int as number;
    this.updateUrl = this.routingService.getRoutePath('diaryentry-update', {
